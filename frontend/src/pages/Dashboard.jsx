...
function WatchlistTab() {
  ...
  useEffect(() => {
    ...
    async function loadWatchlistNews() {
      ...
      try {
        ...
        if (!isMounted) return
        setNewsItems([])
        setNewsError(error.message || '관심종목 뉴스 조회에 실패했습니다.')
      } finally {
        if (isMounted) setNewsLoading(false)
      }
    }
  ...
  return (
    <main className="mx-auto flex max-w-7xl flex-col gap-6">
      <section className="rounded-lg border border-slate-700/80 bg-slate-surface p-5">
        ...
        <div className={useSlider ? 'flex snap-x gap-2 overflow-x-auto pb-2' : 'grid gap-2 md:grid-cols-2 xl:grid-cols-4'}>
          {WATCHLIST_MOCK.map((item) => (
            ...
            >
              <span className="block font-bold">{item.name}</span>
              <span className="mt-1 block text-xs font-mono opacity-70">
      ...
      <section className="rounded-lg border border-slate-700/80 bg-slate-surface p-5">
        <SectionHeader title="관심종목 차트" action={selectedItem?.id} />
        <div className="rounded-lg border border-slate-800 bg-[#0f172a]/70 p-4">
          <MiniSparkline values={WATCH_CHARTS_MOCK[selectedItem?.id]} />
        </div>
        <div className="mt-4 grid gap-3 md:grid-cols-5">
          {[
            ...
            ['계좌종류', selectedItem?.account],
            ['수량', selectedItem?.quantity],
            ['평균단가', selectedItem?.average],
            ['수익률', selectedItem?.change],
          ].map(([label, value]) => (
            <div key={label} className="rounded-lg bg-[#0f172a] p-4">
              <p className="text-xs font-bold text-slate-500">{label}</p>
              <p className="mt-2 font-mono font-bold text-white">{label === '수익률' ? <Rate value={value} /> : value}</p>
            </div>
          ))}
        </div>
      </section>
