-- admin_list_user_token_usage 및 admin_get_user_token_usage RPC 함수를 수정하여
-- 목록에서 최근 사용 모델(recentModel)을 표시하고 상세에서 모델별 사용 통계(byModel)를 추가로 반환합니다.
...
BEGIN
    ...
    usage_rows AS MATERIALIZED (
        SELECT
            ...
            (array_agg(l.model ORDER BY l.created_at DESC) FILTER (WHERE l.model IS NOT NULL AND l.model <> ''))[1] AS recent_model
