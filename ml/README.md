...
현재 코드 기준으로 주식 모델은 `v11`까지, 코인 모델은 `v8`까지 존재합니다.
...
- 신호 모델: `lgbm_crypto_v1.yaml` ~ `lgbm_crypto_v8.yaml`
- 위험 모델: `lgbm_crypto_risk_v1.yaml` ~ `lgbm_crypto_risk_v8.yaml`
...
- 주식 `v11` 모델 파일과 설정은 존재합니다.
- 하지만 자동 수집+학습 preset의 기본 버전은 아직 `v11`이 아닙니다.
- 따라서 "주식 최신 실험 버전"과 "자동화 기본 버전"은 다를 수 있습니다.
...
ml/models/lgbm_crypto_signal_v8.joblib
ml/models/lgbm_crypto_risk_v8.joblib
```
...
- 주식 자동화 기본 preset이 `v11`이라고 적어도 현재 코드와 다릅니다.
